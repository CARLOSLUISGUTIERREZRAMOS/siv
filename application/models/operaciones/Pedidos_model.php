...
//        return $this->db->last_query();
    ...
    function GetAllPedidos($tipo) {
        ...
        switch ($tipo) {
            case 'PEDIDO':
                $estado = array('RQ', 'VT', 'EP', 'ET', 'RL', 'EN');
...
//        $this->db->order_by("P.codigo", "DESC");
    ...
    function ObtenerPedidoDetalleViaje() {
        ...
        $this->db->where_in('PD.estado', 'EP');
        $this->db->where('P.estado', 'RQ');
...
//        $res = $this->db->last_query();
    ...
    function RegistrarPedidoDetalle($data) {
        $this->db->set($data);
        $res_reg_pedido_detalle = $this->db->insert('pedido_detalle');
        return $res_reg_pedido_detalle;
    }
